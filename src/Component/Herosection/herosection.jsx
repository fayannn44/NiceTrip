...
function Hero() {
  ...
  const [activeIndex, setActiveIndex] = useState(null); // 👈 Tambahan: untuk mendeteksi gambar yang diklik
  ...
  // Fungsi klik gambar di mode mobile
  const handleImageClick = (index) => {
    // Kalau diklik lagi, sembunyikan
  ...
  return (
    <section className="relative w-full h-[900px] overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center h-full text-center text-white pb-[380px] px-4">
        <h1 className="text-6xl sm:text-7xl md:text-8xl lg:text-9xl font-extrabold tracking-tighter mb-10 leading-none text-white">
          DISCOVER YOUR
        ...
        <div className="absolute bottom-15 w-full px-4 md:px-10 lg:px-20 z-20">
          <div className="flex flex-nowrap md:grid md:grid-cols-4 gap-4 max-w-7xl mx-auto scroll-smooth scrollbar-hide">
            {cardImages.map((image, index) => (
              <div
                key={index}
                onClick={() => handleImageClick(index)} // 👈 klik gambar
                className="relative w-72 sm:w-80 md:w-full rounded-xl overflow-hidden shadow-2xl transition-all duration-300 group cursor-pointer border-4 border-transparent hover:border-white"
              >
                <div className="transform transition-transform duration-300 group-hover:scale-[1.05]">
                  ...
                  {/* 👇 Tambahan untuk mobile: muncul saat diklik */}
                  {activeIndex === index && (
                    <div className="absolute inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 transition-opacity duration-500 md:hidden">
